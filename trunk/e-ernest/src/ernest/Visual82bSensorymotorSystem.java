...
public class Visual82bSensorymotorSystem extends BinarySensorymotorSystem
{
	...
	public IAct enactedAct(ISchema schema, boolean status)
	{
		...
		int satisfaction = m_satisfaction
				+ schema.resultingAct(status).getSatisfaction();

		// Create the act in episodic memory if it does not exist.

		IAct enactedAct = m_episodicMemory.addAct(label, schema, status,
				satisfaction, Ernest.RELIABLE);
	...
	public void senseMatrix(int[][] matrix)
	{
		...
		m_currentBlueCount = (matrix[0][0] < Ernest.INFINITE ? 1 : 0)
				+ (matrix[1][0] < Ernest.INFINITE ? 1 : 0);
